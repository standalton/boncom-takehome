...
| `ImportWizard` | `src/components/import/ImportWizard.tsx` | 3-step import flow (upload → map columns → preview & commit) with a numbered stepper; built from Card/Select/Badge like the rest of the app. Reached from the sidebar "Import" item; list empty states deep-link with `?target=`. |
